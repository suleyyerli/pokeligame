...
def get_items():
    ...
    cur.execute("SELECT 'card' as type, id, name, rarity, quantity, price FROM cards WHERE user_id = %s", [current_user_id])
    cards = cur.fetchall()
    
    # Récupération des boosters
    cur.execute("SELECT 'booster' as type, id, name, rarity, quantity, price FROM boosters WHERE user_id = %s", [current_user_id])
    boosters = cur.fetchall()
    
    # Récupération des displays
    cur.execute("SELECT 'display' as type, id, name, rarity, quantity, price FROM displays WHERE user_id = %s", [current_user_id])
...
def delete_item(item_type, item_id):
    ...
    try:
        ...
        cur.execute(f"DELETE FROM {table_name} WHERE id = %s AND user_id = %s", (item_id, current_user_id))
        mysql.connection.commit()
...
def update_item(item_type, item_id):
    ...
    try:
        ...
        cur.execute(f"SELECT * FROM {table_name} WHERE id = %s AND user_id = %s", (item_id, current_user_id))
